...
import { Loader2, FileUp, FileCheck2, ArrowDownCircle, ArrowUpCircle, Info } from "lucide-react";
...
import { importTransactions } from "@/app/(dashboard)/finance/actions";
...
export function ImportStatementDialog({ open, onOpenChange, accounts }: ImportStatementDialogProps) {
  ...
  const reset = () => { setFileName(""); setParsed([]); setParseError(null); if (fileRef.current) fileRef.current.value = ""; };
    ...
    <Dialog open={open} onOpenChange={(o) => { onOpenChange(o); if (!o) reset(); }}>
      <DialogContent size="lg">
        <DialogHeader
          ...
          description="Envie o arquivo OFX ou CSV do seu banco. O arquivo é lido no seu navegador — nada é enviado a terceiros."
        />
        <DialogBody className="space-y-4">
          ...
          >
            ...
            <p className="text-[11px] text-muted-foreground">Formatos: .ofx, .csv</p>
            <input
              ref={fileRef}
              type="file"
