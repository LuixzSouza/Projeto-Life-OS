...
import { AlertTriangle, Sun, CalendarClock, CalendarDays, CalendarOff, type LucideIcon } from 'lucide-react';
...
const TONE: Record<Tone, { icon: LucideIcon; chip: string; title: string }> = {
  ...
  none: { icon: CalendarOff, chip: 'bg-muted text-muted-foreground', title: 'text-muted-foreground' },
};
...
function buildSections(tasks: Task[]): Section[] {
  ...
  for (const t of tasks) {
    if (!t.dueDate) { bucket('none', 'Sem data', Number.POSITIVE_INFINITY, 'none').tasks.push(t); continue; }
