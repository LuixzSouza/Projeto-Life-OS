...
const STATUS_CONFIG: Record<string, StatusConfig> = {
    APPLIED: { label: "Inscrito", colorClass: "text-blue-600 bg-blue-500/10 border-blue-500/20", progress: 15, icon: Clock },
    SCREENING: { label: "Triagem", colorClass: "text-purple-600 bg-purple-500/10 border-purple-500/20", progress: 30, icon: Search },
    TEST: { label: "Teste", colorClass: "text-orange-600 bg-orange-500/10 border-orange-500/20", progress: 50, icon: FileCode },
    INTERVIEW: { label: "Entrevista", colorClass: "text-yellow-600 bg-yellow-500/10 border-yellow-500/20", progress: 75, icon: Users },
    OFFER: { label: "Proposta", colorClass: "text-emerald-600 bg-emerald-500/10 border-emerald-500/20", progress: 90, icon: DollarSign },
    ACTIVE: { label: "Contratado", colorClass: "text-indigo-600 bg-indigo-500/10 border-indigo-500/20", progress: 100, icon: CheckCircle2 },
    REJECTED: { label: "Encerrado", colorClass: "text-muted-foreground bg-muted border-border", progress: 0, icon: XCircle },
};
...
function JobListItem({ job, mode }: { job: JobApplication, mode: 'list' | 'grid' }) {
    ...
    if (mode === 'grid') {
        return (
            <Card className="group relative overflow-hidden hover:shadow-lg transition-all border-border bg-card">
                <div className={cn("absolute top-0 left-0 w-full h-1", status.colorClass.split(" ")[0].replace("text-", "bg-"))} />
                <CardContent className="p-5 pt-6">
                    <div className="flex justify-between items-start mb-4">
                        <div className="h-12 w-12 rounded-xl bg-background border flex items-center justify-center shadow-sm">
                            {logo ? <img src={logo} alt={job.company} className="w-6 h-6 object-contain" /> : <Briefcase className="w-5 h-5 text-muted-foreground" />}
                        </div>
                        <Badge variant="outline" className={cn("text-[10px] font-bold uppercase", status.colorClass)}>
                            {status.label}
                        </Badge>
                    </div>
                    
                    <h4 className="font-bold text-lg truncate text-foreground">{job.company}</h4>
                    <p className="text-sm text-muted-foreground truncate mb-4">{job.role}</p>
                    ...
                    <div className="space-y-3">
                        ...
                        <Progress value={status.progress} className="h-1.5 bg-muted" />
                        
                        <div className="flex justify-between items-center pt-2">
                             {job.salary && <span className="text-xs font-mono bg-emerald-500/10 text-emerald-600 px-1.5 py-0.5 rounded">{job.salary}</span>}
                             <div className="flex gap-1 ml-auto">
                                <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-foreground" onClick={() => setIsEditOpen(true)}><Pencil className="h-3.5 w-3.5" /></Button>
    ...
    return (
        <div className="group flex flex-col sm:flex-row items-start sm:items-center gap-4 p-4 bg-card border border-border rounded-xl hover:shadow-md hover:border-primary/30 transition-all mb-3 relative overflow-hidden">
            <div className={cn("absolute left-0 top-0 bottom-0 w-1", status.colorClass.split(" ")[0].replace("text-", "bg-"))} />
            
            <div className="flex items-center gap-4 flex-1 min-w-0">
                <div className="h-12 w-12 rounded-lg bg-background flex items-center justify-center border shrink-0 shadow-sm">
                    {logo ? <img src={logo} alt={job.company} className="w-7 h-7 object-contain" /> : <Briefcase className="h-5 w-5 text-muted-foreground" />}
                </div>
                <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                        ...
                        {job.salary && <span className="flex items-center gap-1 text-emerald-600 font-medium"><DollarSign className="h-3 w-3" /> {job.salary}</span>}
                    </div>
                </div>
            </div>
            ...
            <div className="w-full sm:w-48 flex flex-col gap-1.5 shrink-0">
                <div className="flex justify-between items-center text-xs">
                    <span className={cn("font-bold flex items-center gap-1.5", status.colorClass.split(" ")[0])}>
        ...
        <form action={async (fd) => { 
            ...
            <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                    ...
                    <Input name="company" defaultValue={defaultValues?.company} placeholder="Ex: Google" required className="bg-muted/30 border-border" />
                </div>
                <div className="space-y-2">
                    <Label className="text-xs font-bold text-muted-foreground uppercase flex items-center gap-2">
                        <Users className="h-3.5 w-3.5" /> {type === 'JOB' ? 'Cargo' : 'Serviço'}
                    </Label>
                    <Input name="role" defaultValue={defaultValues?.role} placeholder="Ex: Frontend Engineer" required className="bg-muted/30 border-border" />
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                    ...
                    <Input name="salary" defaultValue={defaultValues?.salary || ""} placeholder="R$ 5.000" className="bg-muted/30 border-border font-mono" />
                </div>
                <div className="space-y-2">
                    <Label className="text-xs font-bold text-muted-foreground uppercase flex items-center gap-2">
                        <Globe className="h-3.5 w-3.5" /> Link da Vaga
                    </Label>
                    <Input name="jobUrl" defaultValue={defaultValues?.jobUrl || ""} placeholder="https://..." className="bg-muted/30 border-border" />
                </div>
            </div>
            ...
            <DialogFooter>
                 <Button type="submit" className="w-full bg-primary text-primary-foreground hover:bg-primary/90 font-bold shadow-md">Salvar Registro</Button>
            </DialogFooter>
        </form>
    );
}
            ...
            <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <Card className="bg-blue-500/5 border-blue-500/20 shadow-sm">
                        <CardContent className="p-4 flex flex-col items-center justify-center text-center">
                            ...
                            <span className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mt-1">Em Andamento</span>
                        </CardContent>
                    </Card>
                ...
                <div className="flex justify-between items-center border-b border-border pb-4 mb-4">
                    <div className="flex items-center gap-2 bg-muted p-1 rounded-lg">
                        <button onClick={() => setViewMode('list')} className={cn("p-1.5 rounded-md transition-all", viewMode === 'list' ? "bg-background shadow text-primary" : "text-muted-foreground hover:text-foreground")}>
                            <List className="h-4 w-4" />
                        </button>
                        <button onClick={() => setViewMode('grid')} className={cn("p-1.5 rounded-md transition-all", viewMode === 'grid' ? "bg-background shadow text-primary" : "text-muted-foreground hover:text-foreground")}>
                    ...
                    <Dialog>
                        <DialogTrigger asChild>
                            <Button className="bg-primary text-primary-foreground hover:bg-primary/90 gap-2 shadow-md h-9 text-xs uppercase font-bold tracking-wide">
                        ...
                        <DialogContent className="max-w-xl">
                            <DialogHeader>
                                <DialogTitle className="text-xl flex items-center gap-2">
                                    <div className="p-2 bg-primary/10 text-primary rounded-lg"><Briefcase className="h-5 w-5"/></div>
                                    Novo Registro
                                </DialogTitle>
                            </DialogHeader>
                            <JobForm type={type} />
                ...
                <div className={cn("space-y-1 pb-4", viewMode === 'grid' && "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 space-y-0")}>
                    {filtered.length === 0 ? (
                        <div className="text-center py-16 border-2 border-dashed border-border rounded-xl col-span-full">
                            <div className="w-16 h-16 bg-muted rounded-full flex items-center justify-center mx-auto mb-3">
                                <Briefcase className="h-8 w-8 text-muted-foreground/50" />
                            </div>
                            <p className="text-muted-foreground font-medium">Nenhum registro encontrado.</p>
                            <p className="text-muted-foreground/70 text-sm">Adicione uma vaga para começar a rastrear.</p>
                        </div>
                    ) : (
