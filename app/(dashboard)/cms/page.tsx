  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <header className="border-b border-border/60 bg-gradient-to-b from-primary/10 via-background/95 to-background">
        <div className="max-w-[1600px] mx-auto p-4 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="h-12 w-12 rounded-2xl bg-primary/15 text-primary flex items-center justify-center shadow-sm">
      ...
      <main className="max-w-[1600px] mx-auto p-4 md:p-8 animate-in fade-in duration-500">
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {sites.length === 0 ? (
            <div className="col-span-full flex flex-col items-center justify-center py-24 border-2 border-dashed border-border/60 rounded-2xl bg-muted/40 text-center">
            ...
            sites.map((site) => (
              <Card
                key={site.id}
                className="group flex flex-col bg-card/80 backdrop-blur-[2px] border-border/60 transition-all hover:shadow-xl hover:-translate-y-0.5"
              >
                {/* HEADER CARD */}
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex gap-1 flex-wrap">
                      {site.pages.slice(0, 3).map((page) => (
                        <span
                          key={page.id}
                          className="px-2 py-0.5 rounded-md bg-muted border border-border/60 text-xs"
                        >
                          /{page.slug}
                        </span>
                      ))}
